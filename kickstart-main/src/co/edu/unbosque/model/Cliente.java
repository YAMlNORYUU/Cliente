	...
	public void run() {
		...
		while (!line.equals("Over")) {
			...
			try {
				...
				vp.getConsola().imprimirString("mensaje recibido:");
				...
				vp.getConsola().imprimirString("El servidor no responde, cerrando coneccion.");
				line = "Over";
				try {
					this.socket.close();
				} catch (IOException e) {
					vp.getConsola().imprimirString("Error cerrando socket: " + e.getMessage());
				}
			} catch (IOException i) {
				vp.getConsola().imprimirString("Error: " + i.getMessage());
			}
		}
		...
		try {
			...
			vp.getConsola().imprimirString(i.getMessage());
		}
